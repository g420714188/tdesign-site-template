...
function renderNav(host, nav, deep = 0) {
  ...
  const isActive = location.pathname === nav.path || location.hash.slice(1) === nav.path || location.pathname.endsWith(nav.path);

  // console.log(location.pathname, nav.path,location.hash.slice(1))
  ...
  if (nav.children && nav.path) {
    return html`
      <div class="TDesign-doc-sidenav-group TDesign-doc-sidenav-group--deep${deep}">
        <a class="TDesign-doc-sidenav-group__title TDesign-doc-sidenav-link  ${isActive ? 'active' : ''}" href="${nav.path}"
